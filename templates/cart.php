...
paypal.Buttons({
    ...
    onApprove: function(data, actions) {
        return actions.order.capture().then(function(details) {
            ...
            fetch('http://localhost/paypal/save_transaction.php', {
                    method: 'POST',
                    headers: {
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        window.location.href =
                            `http://localhost/paypal/success.php?orderID=${data.orderID}&payerID=${data.payerID}&paymentID=${data.paymentID}&details=${encodeURIComponent(JSON.stringify(details))}`;
                    } else {
